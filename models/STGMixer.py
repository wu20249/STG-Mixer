...
class MultiscaleDConv(nn.Module):
    def __init__(self, in_channels, out_channels,
        ...
        super(MultiscaleDConv, self).__init__()
        ...
        self.conv3 = nn.Conv1d(
            in_channels, out_channels,
            kernel_size=3, dilation=dilation_rates[1],
            padding=_same_pad(3, dilation_rates[1]), bias=False
        )
        self.conv5 = nn.Conv1d(
            in_channels, out_channels,
            kernel_size=5, dilation=dilation_rates[2],
            padding=_same_pad(5, dilation_rates[2]), bias=False
        )
    ...
    def forward(self, x):
        ...
        x3 = self.conv3(x)
        x5 = self.conv5(x)
        # reshape back to semantic shape
        D = x1.size(1)
        x1 = x1.view(B, N, D, T)
        x3 = x3.view(B, N, D, T)
        x5 = x5.view(B, N, D, T)
        # y = self.EngergyGated(x1, x3, x5)  # [B, N, D, T]
        return x1,x3,x5
...
class Encoder(nn.Module):
    ...
    def forward(self, case, combine):
        ...
        # Dual-layer T-GCN Aggregation block
        D = self._hidden_dim
        ...
        # BiLSTM × L
        x33 = WS.permute(0, 1, 3, 2).contiguous()
        x33 = x33.view(B * N, T, D)
        w_out, _ = self.BiLSTM[0](x33)
        w_out = w_out[:, -1, :]
        w_out = w_out.view(B, N, -1)  # [B, N, 2D]
